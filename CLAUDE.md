...
> **Última actualización:** 19 junio 2026 — **Acta oficial GOR-F-084 pulida + barrido de release (6 agentes)**. Acta DOCX con saneo de textos (`api/src/lib/actaSaneado.js`), **logo SENA real incrustado** (`api/assets/sena-logo.png`) y nota de revisión siempre visible. Smoke test de actas en 5 fichas/competencias (1/2/3 RAPs) ✅. Auditoría de release con 6 agentes (informes en `docs/auditoria-release/`): se cerró **1 bug P0 de fuga de datos cross-tenant** (`confirm-native`), 4 P1 (IDOR matching, errorHandler global, foroRating attempts→1, modal mapeo isError) y 5 fixes mobile. Veredicto: **GO para push, NO-GO de despliegue** (falta infra: rotar OPENROUTER_API_KEY, ALLOWED_ORIGIN, VPS, TLS). Diseño: sobrio y vendible, solo retoque mínimo. "Mensajes en error" = envíos parciales 98% (etiquetado pesimista, no falla). Matching IA escribe RapEvidenciaRel directo = excepción consciente a regla #8. Ver `docs/auditoria-release/00-CONSOLIDADO.md`.
> _(9 jun 2026, tarde: **MATCHING IA AUTOMÁTICO funcionando**: 17 competencias mapeadas en un comando, RapEvidenciaRel 477→2147. Pruebas multi-tenant y flujo real técnico OK. Ver §14.)_
...
**AMPLIACIÓN (pedida por el usuario la misma noche) — filtros automáticos + programación:**

5. **Filtros reales de destinatarios (cero selección manual):** el instructor NO debería marcar aprendices uno a uno. Filtros tipo: "todos los que tengan ≥1 evidencia pendiente (de las evidencias seleccionadas)", "los que tengan desaprobadas", "los que nunca han entrado" (ultimoAcceso). El front resuelve el filtro → arma los destinatarios solo con esos. Flujo deseado: elegir filtro + plantilla borrador → **"pum, envía a esos"** sin entrar aprendiz por aprendiz.
6. **Mensajes programados/recurrentes:** poder programar el envío ("cada X días/semanas, a tal hora") tanto por canal **Zajuna** como por **email (SMTP/Gmail)**. Implementación sugerida: BullMQ ya soporta repeatable jobs (igual que el autoScan cada 3h en `lib/queue.js`) — nuevo modelo Prisma `MensajeProgramado` (userId, fichaId, canal, plantilla, filtro, cron/intervalo, activo, lastRunAt) + worker que al disparar recalcula el filtro EN ESE MOMENTO (los pendientes de hoy, no los de cuando se programó) y encola el envío normal. UI: sección "Programados" en MensajesPage con crear/pausar/borrar.
   - ⚠️ Guardas: respetar idempotencia (no doble envío si el worker reintenta), tope de frecuencia razonable (no spamear aprendices), y log en historial de cada corrida.
